...
func getShort(id string) (string, error) {
	// Load Bit.ly URL shortener config data
	
	apiKey := os.Getenv("BITLY_ACCESS_TOKEN")
	if apiKey == "" {
		return "", errors.New("Can't load Bit.ly API Key from ENV")
	}


	firebasedynamiclinksService, err := firebasedynamiclinks.New(oauthHttpClient)

	firebasedynamiclinksService.ShortLinks.
